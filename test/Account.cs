        ...
        public  void ChangeData(string typeoperation,Account account)
        {
            ...
            if (typeoperation == "Сдать")
            {
...

                {
                    foreach (var v in ss1)
                    {
                        ...
                        {
                        new Account { Id = listI.Count , Name = v.Name,Age = v.Age,Country=v.Country,Phone = v.Phone, Email =  v.Email, Order = v.Order, Status_Order = true,DateClose=DateTime.Now.ToString() }
                        };
            ...
            if (typeoperation == "Взять в аренду")
            {
...

                {
                    var NewRecord = new List<Account>()
                    {
                    new Account { Id = listI.Count , Name = account.Name,Age = account.Age,Country=account.Country,Phone = account.Phone, Email =  account.Email, Order = account.Order, Status_Order = false,DateOpen =DateTime.Now.ToString(),}
                    };
                    foreach (var k in NewRecord)
                    {
                        writer.WriteLine(k.ToExcel());
